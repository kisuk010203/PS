...
bool intersection(seg a, seg b){
    int ccw1 = ccw(a.first, a.second, b.first)*ccw(a.first, a.second, b.second);
    int ccw2 = ccw(b.first, b.second, a.first)*ccw(b.first, b.second, a.second);
...
int main(int argc, char const *argv[]){
    ...
    for(int i=0; i<N; i++){
        ll sx, sy, ex, ey, w;
        cin >> sx >> sy >> ex >> ey >> w;
        lines[i] = make_pair(make_pair(make_pair(sx, sy), make_pair(ex, ey)), w);
    }
