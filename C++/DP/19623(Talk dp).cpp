...
int main() {
    ...
    for (int i = 1; i < num; i++) {
        ...
        if (get<0>(talk_vec[i]) >= get<1>(talk_vec[end]))
            compat[i] = end;
        else if (get<0>(talk_vec[i]) >= get<1>(talk_vec[start]))
            compat[i] = start;
        else
            compat[i] = -1;
    }
    ...
    for (int i = 1; i < num; i++) {
        if (compat[i] == -1)
            dp[i] = max(dp[i - 1], get<2>(talk_vec[i]));
        else
            dp[i] = max(dp[i - 1], dp[compat[i]] + get<2>(talk_vec[i]));
    }
    cout << dp[num - 1];
}
