...
void setcar(int i, int j, int ans, int mode) {
    ...
    if (j != i - 1) {
        ...
        for (int k = 0; k < j; k++) {
            int temp = arr[j][k][mode] +
                       abs(problem[i][0][mode] - problem[k][0][mode]) +
...
int main() {
    ...
    for (int i = 1; i < w + 1; i++) {
        int r, c;
        cin >> problem[i][0][0] >> problem[i][1][0];
        ...
        for (int j = 0; j < i; j++) {
            if (j != i - 1) {
                arr[i][j][0] = arr[i - 1][j][0] +
                               abs(problem[i - 1][0][0] - problem[i][0][0]) +
                               abs(problem[i - 1][1][0] - problem[i][1][0]);
                arr[i][j][1] = arr[i - 1][j][1] +
                               abs(problem[i - 1][0][1] - problem[i][0][1]) +
                               abs(problem[i - 1][1][1] - problem[i][1][1]);
            } else {
                arr[i][j][0] = INF;
                arr[i][j][1] = INF;
                int temp, tempa;
                for (int k = 0; k < j; k++) {
                    temp = arr[j][k][0] +
                           abs(problem[i][0][0] - problem[k][0][0]) +
                           abs(problem[i][1][0] - problem[k][1][0]);
                    tempa = arr[j][k][1] +
                            abs(problem[i][0][1] - problem[k][0][1]) +
