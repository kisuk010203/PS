...
int main() {
    ...
    queue<tup> que;
    que.push(make_tuple(start_x, start_y, 0, 0));
    ...
    while (!que.empty()) {
        ...
        if (maze[x][y] == '#')
            ...
            if ((keys & (1 << (maze[x][y] - 'A'))) == 0)
                continue;
        }
        for (int i = 0; i < 4; i++) {
            if (in_bounds(x + x_diff[i], y + y_diff[i]) &&
                (visited[x + x_diff[i]][y + y_diff[i]][keys] == 0) &&
                (maze[x + x_diff[i]][y + y_diff[i]] != '#')) {
                que.push(
                    make_tuple(x + x_diff[i], y + y_diff[i], cnt + 1, keys));
