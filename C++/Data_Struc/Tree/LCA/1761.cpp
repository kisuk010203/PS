    ...
    for (auto child : adj_list[point]) {
        if (!visited[child.first]) {
            ...
            while (par[par[child.first][exp - 1].first][exp - 1].first != -1) {
                par[child.first][exp] = make_pair(
                    par[par[child.first][exp - 1].first][exp - 1].first,
                    par[par[child.first][exp - 1].first][exp - 1].second +
                        par[child.first][exp - 1].second);
                exp++;
            }
...
int main() {
    ...
    while (M--) {
        ...
        if (v1 != v2)
            res += (par[v1][0].second + par[v2][0].second);
        cout << res << "\n";
    }
}
