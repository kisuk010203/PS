...
int valid(int r, int c, int cnt){
    ...
    for(int i=1; i<=cnt; i++){
        ...
        if((r+c == temp_r + temp_c) || (r-c == temp_r - temp_c)) return 0;
    }
    return 1;
}
p next_idx(p idx){
    int r = idx.first, c = idx.second;
    if(c < n-2) return make_pair(r, c+2);
    else if(r<n-1) return make_pair(r+1, (c+1)%2);
...
void backtracking(int r, int c, int cnt){
    if(cnt == 0){
        ...
        int flag = 1;
        for(p idx = next_idx(make_pair(r, c)); idx.first>=0; idx = next_idx(idx)){
            if(valid(idx.first, idx.second, cnt)){
                bishop[cnt+1] = idx;
                flag = 0;
                backtracking(idx.first, idx.second, cnt+1);
