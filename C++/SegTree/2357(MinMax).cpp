...
void init(int node, int start, int end) {
    ...
    max_segtree[node] = max(max_segtree[2 * node], max_segtree[node * 2 + 1]);
    return;
}
pl find_ans(int node, int start, int end, int left, int right) {
    ...
    int mid = (start + end) / 2;
    pl front = find_ans(2 * node, start, mid, left, right),
       back = find_ans(2 * node + 1, mid + 1, end, left, right);
    return make_pair(min(front.first, back.first),
                     max(front.second, back.second));
}
int main() {
    cin.tie(0);
    cout.tie(0);
