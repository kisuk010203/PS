...
ll init_sum(ll node, ll left, ll right){
    if(left == right) return sumsegtree[node] = arr[left];
    ll mid = (left+right)/2;
    return sumsegtree[node] = init_sum(2*node, left, mid) + init_sum(2*node+1, mid+1, right);
}
ll find_min(ll node, ll left, ll right, ll start, ll end){
    if(left > end || start > right) return 100000;
...
ll getsum(ll node, ll left, ll right, ll start, ll end){
    ...
    if(start <= left && right <= end) return sumsegtree[node];
    ll mid = (left+right)/2;
    return getsum(2*node, left, mid, start, end) + getsum(2*node+1, mid+1, right, start, end);
}
