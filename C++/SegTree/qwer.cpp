...
#include <vector>
using namespace std;
void init(vector<long long>& a, vector<long long>& tree, int node, int start,
          int end) {
    if (start == end) {
        tree[node] = a[start];
    } else {
...
long long query(vector<long long>& tree, vector<long long>& lazy, int node,
    ...
    long long lsum =
        query(tree, lazy, node * 2, start, (start + end) / 2, left, right);
