...
ll init(ll* segtree, ll* arr, int node, int start, int end) {
    ...
    int mid = (start + end) / 2;
    ll front = init(segtree, arr, 2 * node, start, mid),
       back = init(segtree, arr, 2 * node + 1, mid + 1, end);
...
ll get_pro(ll* segtree, ll node, int start, int end, int left, int right) {
    if (start > right || end < left)
        ...
        return segtree[node];
    int mid = (start + end) / 2;
    return (get_pro(segtree, node * 2, start, mid, left, right) *
            get_pro(segtree, node * 2 + 1, mid + 1, end, left, right)) %
           MOD;
}
int main() {
    cin >> n >> m >> k;
