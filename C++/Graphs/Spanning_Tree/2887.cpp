...
int kruskal() {
    ...
    for (int u = 0; u < V - 1; u++) {
        edges.push_back(make_pair(abs(get<1>(x[u]) - get<1>(x[u + 1])),
                                  make_pair(get<0>(x[u]), get<0>(x[u + 1]))));
        edges.push_back(make_pair(abs(get<2>(y[u]) - get<2>(y[u + 1])),
                                  make_pair(get<0>(y[u]), get<0>(y[u + 1]))));
        edges.push_back(make_pair(abs(get<3>(z[u]) - get<3>(z[u + 1])),
                                  make_pair(get<0>(z[u]), get<0>(z[u + 1]))));
    }
